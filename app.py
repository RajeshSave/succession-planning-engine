...
with st.sidebar:
    ...
    st.markdown(f"<div style='color:#6EE7B7;font-size:0.8rem;margin-top:6px'>✓ {n_loaded}/7 files loaded</div>",
                unsafe_allow_html=True)
...
with tab4:
    if "org" not in data:
        ...
        for node in G.nodes():
            ...
            node_hover.append(f"<b>{node}</b><br>Grade: {g}<br>LPS: {lps_v:.1f if not pd.isna(lps_v) else 'N/A'}<br>{'⭐ Critical Role' if is_cr else ''}")
            ...
            if len(emp_cp) > 0:
                ...
                <div class="card">
                  ...
                  <div style="font-size:0.78rem;color:#374151">
                    ...
                    <b>Total Promos:</b> {int(e['Total Promotions (Career)'])} &nbsp;|&nbsp; <b>LPS:</b> {lps_v:.1f}
                  </div>
                </div>""", unsafe_allow_html=True)
